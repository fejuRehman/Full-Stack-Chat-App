    ...
    <div style={{backgroundImage : `url(${backgroundImage})`}} className='bg-no-repeat bg-cover' >
               ...
               <Link to={"/"} className='lg:hidden'>
                ...
                <div>
                  ...
                  <p className='-my-2 text-sm'>
                     {
                      dataUser.online ? <span className='text-secondary'>online</span>: <span className='text-slate-400'>offline</span>
                     }
                  </p>
                </div>
            </div>
        ...
        <section className='h-[calc(100vh-128px)] overflow-x-hidden overflow-y-scroll relative bg-slate-200 bg-opacity-50 '>
            ...
            <div className='flex flex-col gap-2 py-2 mx-2'ref={currentMessage}>
               {
                 allMessage!==null  && allMessage.map((msg,index)=>{
                     return (
...

            {
              message?.imageUrl && (
                <div className='w-full h-full sticky bottom-0 bg-slate-700 bg-opacity-30 flex justify-center items-center rounded overflow-hidden'>
                   <div className='w-fit p-2 absolute top-0 right-0 cursor-pointer hover:text-red-600' onClick={handleUploadClearImage} >                      
                           <IoMdClose size={30} /> 
                   </div>
            ...
            {/* upload video display  */}
            {
              message?.videoUrl && (
                <div className='w-full h-full sticky bottom-0 bg-slate-700 bg-opacity-30 flex justify-center items-center rounded overflow-hidden'>
                   <div className='w-fit p-2 absolute top-0 right-0 cursor-pointer hover:text-red-600' onClick={handleUploadClearVideo} >                      
                           <IoMdClose size={30} />                    
                   </div>
        ...
        {/* send message  */}
        <section className='relative h-16 bg-white flex items-center px-4'>
            <div >
            ...
            {/* {input box } */}
            <form className='w-full h-full flex gap-2' onSubmit={handleChangeMessage} >
